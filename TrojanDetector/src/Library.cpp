...
void Library::addEntry(std::vector<std::string> &tokens) {
	...
	libEntries.push_back(libraryEntry(id, dT, offSet, cord, tokens[4] + tokens[5] + tokens[6] + tokens[7]));
}
Coordinate Library::getCoordinate(std::string str) {
	...
	std::string strX = str.substr(index1, index2 - index1 - 1);
	std::string strY = str.substr(index2, str.length());
